...
namespace Data
{
    ...
    public class WMORootFile : WMOFile
    {
        ...
        protected override void parseFile(string mpqfile)
        {
            MpqStream ms = DataManager.MPQLoader.OpenFile(mpqfile);
            if (ms == null)
                throw new Exception(String.Format("Unable to load {0}", mpqfile));
            ...
            while (pos < ms.Length)
            {
                ...
                if (tempHeader.Is("MOHD"))  // WMO Root Header
                {
                    ...
                    mohd.c1 = new Vector3(bin.ReadSingle(), bin.ReadSingle(), bin.ReadSingle());
                    mohd.c2 = new Vector3(bin.ReadSingle(), bin.ReadSingle(), bin.ReadSingle());
                ...
                if (tempHeader.Is("MOGI"))  // Group information for WMO groups.
                {
                    ...
                    for (int i = 0; i < num; i++)
                    {
                        ...
                        groupInfo[i].c1 = new Vector3(bin.ReadSingle(), bin.ReadSingle(), bin.ReadSingle()); ;
                        groupInfo[i].c2 = new Vector3(bin.ReadSingle(), bin.ReadSingle(), bin.ReadSingle()); ;
                ...
                if (tempHeader.Is("MOPV"))  // Portal vertices. Portals are (always?) rectangles that specify where doors or entrances are in a WMO.
                {
                    ...
                    for (int i = 0; i < num; i++)
                    {
                        portalVertices[i].c1 = new Vector3(bin.ReadSingle(), bin.ReadSingle(), bin.ReadSingle());
                        portalVertices[i].c2 = new Vector3(bin.ReadSingle(), bin.ReadSingle(), bin.ReadSingle());
                        portalVertices[i].c3 = new Vector3(bin.ReadSingle(), bin.ReadSingle(), bin.ReadSingle());
                        portalVertices[i].c4 = new Vector3(bin.ReadSingle(), bin.ReadSingle(), bin.ReadSingle());
                    }

...
namespace Data
{
    ...
    public class DBCFile
    {
        ...
        public int getSizeofDBC() //for use with arrays(lists)
        {
            return data.Length;
        }
        ...
        {
            MpqStream fs = DataManager.MPQLoader.OpenFile(mpqfile);

            if (fs == null)
                return;

            byte[] rawData = new byte[fs.Length];
            int Read = fs.Read(rawData, 0, (int)fs.Length);

            if (Read < fs.Length)
            {
                throw new Exception(string.Format("Unable to read entire DBC File! Read: {0} Total Size: {1}", Read, fs.Length));
            }

            fs = null; // free up old file

            BinaryReader bin = new BinaryReader(new MemoryStream(rawData));
            ...
            //{
            // /   bin.ReadByte();
            //    Console.WriteLine("Warning, successfully read 1 byte beyond what should have been the end of the file! Investigate {0}.", mpqfile);
            //}

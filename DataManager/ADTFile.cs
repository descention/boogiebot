...
namespace Data
{
    /// <summary>ADT File. Parses and stores a maptile, and its 256 map chunks.</summary>
    public class ADTFile
    {
        ...
        public ADTFile(String mapname, int x, int y)
        {
            string FullPath = String.Format("World\\Maps\\{0}\\{0}_{1}_{2}.adt", mapname, x, y);
            parseFile(FullPath);
        }

        private void parseFile(string mpqfile)
        {
            MpqStream ms = DataManager.MPQLoader.OpenFile(mpqfile);

            if (ms == null)
                throw new Exception(String.Format("Unable to find ADT: {0}", mpqfile)); 
            ...
            while (pos < ms.Length)
            {
                ...
                if (tempHeader.Is("MHDR"))  // ADT File Header
                {
                    ...
                    mhdr.offsFlightBoundary = bin.ReadUInt32();
                    mhdr.offsMH2O = bin.ReadUInt32();
                ...
                if (tempHeader.Is("MDDF"))  // Placement information for doodads (M2 models). 
                {
                    ...
                    for(int i = 0 ; i<num ; i++)
                    {
                        ...
                        doodadLocations[i].coord = new Vector3(bin.ReadSingle(), bin.ReadSingle(), bin.ReadSingle());
                    }
                ...
                if (tempHeader.Is("MODF"))  // Placement information for WMOs.
                {
                    ...
                    for (int i = 0; i < num; i++)
                    {
                        ...
                        wmoLocations[i].coord = new Vector3(bin.ReadSingle(), bin.ReadSingle(), bin.ReadSingle());
                        wmoLocations[i].orientation = new Vector3(bin.ReadSingle(), bin.ReadSingle(), bin.ReadSingle());
                        wmoLocations[i].coord2 = new Vector3(bin.ReadSingle(), bin.ReadSingle(), bin.ReadSingle());
                        wmoLocations[i].coord3 = new Vector3(bin.ReadSingle(), bin.ReadSingle(), bin.ReadSingle());
            ...
            // End of the file !
           // BenderCore.Log(LogType.SystemDebug, "ATDFile: done!", mpqfile);
        }
        
        // Read in an MCNK chunk at supplied offset.
        private MCNK parseMapChunk(MpqStream ms, uint offset, uint size)
        {
            BinaryReader bin = new BinaryReader(ms);
            ms.Position = offset;
        ...
        private void parseMapChunkSubChunks(ref MCNK mapChunk, MpqStream ms, long lastpos)
        {
            BinaryReader bin = new BinaryReader(ms);
        ...
        private struct MHDR
        {
            ...
            public UInt32 offsFlightBoundary;
            public UInt32 offsMH2O;
            public UInt32 pad3;
            public UInt32 pad4;
            public UInt32 pad5;
            public UInt32 pad6;

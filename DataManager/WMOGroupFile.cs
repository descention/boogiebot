...
namespace Data
{
    ...
    public class WMOGroupFile : WMOFile
    {
        ...
        public Vector3[] vertices;      // Triangle Vertex Locations
        public Vector3[] normals;           // Triangle Normals
        ...
        protected override void parseFile(string mpqfile)
        {
            ...
            if (ms == null)
                throw new Exception(String.Format("Unable to open file: {0}", mpqfile));
            ...
            while (pos < ms.Length)
            {
                ...
                if (tempHeader.Is("MOGP"))  //  WMO Group Header
                {
                    ...
                    mogp.c1 = new Vector3(bin.ReadSingle(), bin.ReadSingle(), bin.ReadSingle());
                    mogp.c2 = new Vector3(bin.ReadSingle(), bin.ReadSingle(), bin.ReadSingle());
                ...
                if (tempHeader.Is("MOVT"))  // Vertices chunk.
                {
                    ...
                    for (int i = 0; i < num; i++)
                        vertices[i] = new Vector3(bin.ReadSingle(), bin.ReadSingle(), bin.ReadSingle());
                ...
                if (tempHeader.Is("MONR"))  // Normals.
                {
                    ...
                    for (int i = 0; i < num; i++)
                        normals[i] = new Vector3(bin.ReadSingle(), bin.ReadSingle(), bin.ReadSingle());

                    continue;
                }

                if (tempHeader.Is("MOTV"))  // Texture Locations.
                {
                    // Not needed.
                    continue;
                }
                ...
                if (tempHeader.Is("MLIQ"))  // Specifies liquids inside WMOs.
                {
                    ...
                    mliq.coord = new Vector3(bin.ReadSingle(), bin.ReadSingle(), bin.ReadSingle());
